...
async function loadStudentProfileData(studentId) {
  ...
  const [allProgress, memorization, notes, quranEntries, topicEntries, scheduleSlots, courseLogs, diplomas] = await Promise.all([
  ...
  return { progressMap, memorization, notes, quranEntries, topicEntries, scheduleSlots, courseLogs, diplomas };
}
...
// -----------------------------------------------------------------------
// -----------------------------------------------------------------------
// Suivi détaillé Athkar / Fiqh / Tafsir — même principe que le Coran :
// thème/dhikr étudié, daté, librement saisi par l'enseignante.
// -----------------------------------------------------------------------
...
function openTopicTrackingModal(studentId, subjectKey, entry) {
  ...
  openModal(`
    ...
    <div class="field" style="margin-bottom:12px"><label>${t("topicThemeLabel")}</label><input id="tpTheme" value="${e.theme}" placeholder="${subjectKey === "athkar" ? t("topicAthkarPlaceholder") : subjectKey === "fiqh" ? t("topicFiqhPlaceholder") : subjectKey === "sira" ? t("topicSiraPlaceholder") : t("topicTafsirPlaceholder")}" /></div>
