...
function renderProfilePedagogicalSummary(student, data, lang) {
  ...
  const juzDone = JUZ_LIST.filter((j) => data.memorization[j.id]?.completed).length;
...
function renderProfileStats(student, data) {
  ...
  const juzDone = JUZ_LIST.filter((j) => data.memorization[j.id]?.completed).length;
  const attended = (data.courseLogs || []).filter((l) => l.studentId === student.id && l.attendance === "present").length;
  const absences = (data.courseLogs || []).filter((l) => l.studentId === student.id && l.attendance === "absent").length;
