...
function openDB() {
  ...
  _dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      // oldVersion >= 4 : rien à faire, schéma déjà à jour.
      finishFreshSchema();
    };
...
const DB = {
  ...
  async exportAll() {
    const [students, progress, badges, diplomas, settings, memorization] = await Promise.all([
      DB.getAllStudents(),
      idbGetAll("progress"),
      idbGetAll("badges"),
      idbGetAll("diplomas"),
    ...
    return { version: DB_VERSION, exportedAt: new Date().toISOString(), students, progress, badges, diplomas, settings, memorization };
  },
