...
//   surahChecklist { studentId (clé), surahIds: [numéros de sourates
//                acquises], updatedAt }
//
...
// =============================================================================
// PHOTOS DE PROFIL DES ÉLÈVES — fonctions réutilisables partout dans l'app.
// Les photos sont stockées en base64 directement sur le profil de l'élève
// (store "students"), donc incluses automatiquement dans l'export/import
// de sauvegarde, sans rien changer à ces mécanismes.
// =============================================================================

// Initiales de repli si aucune photo n'est enregistrée.
...
// Retourne le HTML d'un avatar rond (photo si disponible, sinon initiales
// sur fond coloré). Utilisable partout où un nom d'élève est affiché.
function avatarHtml(name, photo, size) {
  size = size || 36;
  if (photo) {
    return `<img src="${photo}" alt="" style="width:${size}px;height:${size}px;min-width:${size}px;border-radius:50%;object-fit:cover;flex-shrink:0" />`;
  }
  const initials = avatarInitials(name);
  const fontSize = Math.round(size * 0.4);
  return `<div style="width:${size}px;height:${size}px;min-width:${size}px;border-radius:50%;background:var(--color-primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:${fontSize}px;font-weight:700;flex-shrink:0">${initials}</div>`;
}

// Redimensionne une photo choisie (caméra ou galerie) à une taille
// raisonnable avant de la stocker, pour éviter d'alourdir la base locale.
  ...
  _dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = (e) => {
      ...
      const finishFreshSchema = () => {
        ...
        if (!db.objectStoreNames.contains("studentNotes")) db.createObjectStore("studentNotes", { keyPath: "id" });
        if (!db.objectStoreNames.contains("quranTracking")) db.createObjectStore("quranTracking", { keyPath: "id" });
        if (!db.objectStoreNames.contains("topicTracking")) db.createObjectStore("topicTracking", { keyPath: "id" });
        if (!db.objectStoreNames.contains("alphabetTracking")) db.createObjectStore("alphabetTracking", { keyPath: "id" });
        if (!db.objectStoreNames.contains("surahChecklist")) db.createObjectStore("surahChecklist", { keyPath: "studentId" });
      };
      ...
      // oldVersion === 8 : ajout de la fiche complète élève (studentNotes :
      // carnet de suivi pédagogique ; quranTracking : suivi détaillé
      // sourate/versets).
      // oldVersion === 9 : ajout du suivi détaillé Athkar/Fiqh/Tafsir
      // (topicTracking), pour les relevés bimestriels détaillés.
      // oldVersion === 10 : ajout du suivi de l'alphabet arabe
      // (alphabetTracking), pour les élèves niveau 1 qui n'ont pas encore
      // commencé le suivi sourate/verset — nouveau, rien à migrer.
      // oldVersion === 11 : ajout de la checklist "sourates déjà acquises"
      // (surahChecklist), pour marquer en une fois ce qu'un élève maîtrise
      // déjà en arrivant (Juz Amma) — nouveau, rien à migrer.
      // oldVersion >= 11 : rien d'autre à faire, schéma déjà à jour.
      finishFreshSchema();
    };
...
const DB = {
  ...
  async deleteStudentCascade(studentId) {
    ...
    // Checklist de sourates déjà acquises
    await idbDelete("surahChecklist", studentId).catch(() => {});
  ...
  // Toute la mémorisation, tous élèves confondus — pour le suivi global des
  // révisions dans l'espace enseignant.
  getAllMemorizationEveryone: () => idbGetAll("memorization"),
  saveJuzProgress: (studentId, juzId, data) =>
    idbPut("memorization", { key: `${studentId}::${juzId}`, studentId, juzId, updatedAt: Date.now(), ...data }),

  // Enregistre une session de révision (muraja'a) pour un Juz' déjà
  // mémorisé — distinct de la mémorisation initiale. Ne touche jamais au
  // champ "completed".
  async logJuzRevision(studentId, juzId) {
    const key = `${studentId}::${juzId}`;
    const existing = await idbGet("memorization", key);
    return idbPut("memorization", {
      ...(existing || { key, studentId, juzId, completed: true }),
      lastRevisedAt: Date.now(),
      revisionCount: ((existing && existing.revisionCount) || 0) + 1,
      updatedAt: Date.now(),
    });
  },
  ...
  // --- carnet de suivi pédagogique (fiche complète élève) : notes datées
  // libres (points forts, difficultés, remarques, objectifs, devoirs) ---
  getAllStudentNotes: (studentId) => idbGetAll("studentNotes").then((rows) => rows.filter((r) => r.studentId === studentId)),
  saveStudentNote: (note) => idbPut("studentNotes", note),
  deleteStudentNote: (id) => idbDelete("studentNotes", id),

  // --- suivi détaillé du Coran (fiche complète élève) : sourate, versets
  // début/fin, dernier verset atteint, statut terminé/à poursuivre ---
  getAllQuranTracking: (studentId) => idbGetAll("quranTracking").then((rows) => rows.filter((r) => r.studentId === studentId)),
  saveQuranTracking: (entry) => idbPut("quranTracking", entry),
  deleteQuranTracking: (id) => idbDelete("quranTracking", id),

  // --- suivi détaillé Athkar / Fiqh / Tafsir (fiche complète élève) :
  // thème/dhikr étudié, daté — sert de base aux relevés bimestriels détaillés
  // envoyés aux parents (au-delà des simples cases "séance validée"). ---
  getAllTopicTracking: (studentId) => idbGetAll("topicTracking").then((rows) => rows.filter((r) => r.studentId === studentId)),
  saveTopicTracking: (entry) => idbPut("topicTracking", entry),
  deleteTopicTracking: (id) => idbDelete("topicTracking", id),

  // --- suivi de l'alphabet arabe (fiche complète élève) : pour les élèves
  // niveau 1 qui n'ont pas encore commencé le suivi sourate/verset —
  // lettres travaillées, dernière lettre atteinte, statut de la leçon. ---
  getAllAlphabetTracking: (studentId) => idbGetAll("alphabetTracking").then((rows) => rows.filter((r) => r.studentId === studentId)),
  saveAlphabetTracking: (entry) => idbPut("alphabetTracking", entry),
  deleteAlphabetTracking: (id) => idbDelete("alphabetTracking", id),

  // --- checklist "sourates déjà acquises" (fiche complète élève) : une
  // seule entrée par élève (pas d'historique daté), contenant la liste des
  // numéros de sourates (Juz Amma) que l'élève maîtrisait déjà en arrivant
  // ou a acquises depuis. Se coche/décoche d'un tapotement, sans modale. ---
  getSurahChecklist: (studentId) => idbGet("surahChecklist", studentId),
  saveSurahChecklist: (record) => idbPut("surahChecklist", record),

  // --- sauvegarde / restauration complète (tous les profils) ---
  async exportAll() {
    const [students, progress, badges, diplomas, settings, memorization, billing, families, registrations, schedule, courseLogs, studentNotes, quranTracking, topicTracking, alphabetTracking, surahChecklist] = await Promise.all([
      DB.getAllStudents(),
      idbGetAll("progress"),
      idbGetAll("badges"),
      idbGetAll("diplomas"),
      ...
      idbGetAll("studentNotes"),
      idbGetAll("quranTracking"),
      idbGetAll("topicTracking"),
      idbGetAll("alphabetTracking"),
      idbGetAll("surahChecklist"),
    ]);
    return { version: DB_VERSION, exportedAt: new Date().toISOString(), students, progress, badges, diplomas, settings, memorization, billing, families, registrations, schedule, courseLogs, studentNotes, quranTracking, topicTracking, alphabetTracking, surahChecklist };
  },
  async importAll(data) {
    ...
    if (Array.isArray(data.studentNotes)) for (const n of data.studentNotes) await idbPut("studentNotes", n);
    if (Array.isArray(data.quranTracking)) for (const q of data.quranTracking) await idbPut("quranTracking", q);
    if (Array.isArray(data.topicTracking)) for (const tp of data.topicTracking) await idbPut("topicTracking", tp);
    if (Array.isArray(data.alphabetTracking)) for (const al of data.alphabetTracking) await idbPut("alphabetTracking", al);
    if (Array.isArray(data.surahChecklist)) for (const sc of data.surahChecklist) await idbPut("surahChecklist", sc);
