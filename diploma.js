...
// Construit un VRAI lien de vérification (au lieu d'un simple texte interne).
// Comme cette app fonctionne 100% en local/hors-ligne (aucun serveur), le
// lien encode directement les informations du document dans son fragment
// d'URL (#/verify/...) : ainsi, quiconque scanne le QR code — même sur un
// autre téléphone que celui de l'enseignante — voit une page de
// vérification avec les informations du document, sans avoir besoin
// d'accéder à une base de données distante qui n'existe pas.
function buildVerifyUrl(payload) {
  const encoded = encodeURIComponent(JSON.stringify(payload));
  return `${location.origin}${location.pathname}#/verify/${encoded}`;
}
...
async function buildDiplomaPdf(opts) {
  ...
  const qrDataUrl = await getQrDataUrl(buildVerifyUrl({
    type: "diploma", num: opts.certNumber, name: opts.studentName,
    title: opts.achievementTitle, date: opts.issuedAtLabel, lang: opts.lang,
  }));
  if (qrDataUrl) {
    const qrSize = 20;
...
async function buildKhatmDiplomaPdf(opts) {
  ...
  const qrDataUrl = await getQrDataUrl(buildVerifyUrl({
    type: "khatm", num: opts.certNumber, name: opts.studentName,
    date: opts.issuedAtLabel, lang: opts.lang,
  }));
  if (qrDataUrl) {
    const qrSize = 22;
...
async function buildBulletinPdf(opts) {
  ...
  const qrDataUrl = await getQrDataUrl(buildVerifyUrl({
    type: "bulletin", num: opts.bulletinNumber, name: opts.studentName,
    date: opts.generatedAtLabel, lang: opts.lang,
  }));
  if (qrDataUrl) {
    const qrSize = 20;
