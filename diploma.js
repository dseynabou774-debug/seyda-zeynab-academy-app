...
const DIPLOMA_LABELS = {
  fr: {
    ...
    signatureRole: "Direction pédagogique", verify: "Vérification en ligne",
  ...
  en: {
    ...
    signatureRole: "Academic Direction", verify: "Online verification",
  ...
  it: {
    ...
    signatureRole: "Direzione didattica", verify: "Verifica online",
...
async function renderStyledTextImage(text, opts) {
  ...
  ctx.fillText(text, padX, padX * 0.7 + ascent);
...
function drawSeal(doc, cx, cy, radius, lines) {
  ...
  for (let i = 0; i < dots; i++) {
    ...
    const lx = cx + Math.cos(angle) * radius * 0.92;
    const ly = cy + Math.sin(angle) * radius * 0.92;
    doc.setFillColor(...GOLD);
    doc.circle(lx, ly, radius * 0.045, "F");
  }
...
async function buildDiplomaPdf(opts) {
  ...
  // Fond + texture discrète
  doc.setFillColor(...CREAM_BG);
  doc.rect(0, 0, W, H, "F");
  drawTexture(doc, 16, 16, W - 32, H - 32);

  // Silhouettes de mosquée, très discrètes, de part et d'autre
  drawMosqueSilhouette(doc, 24, 128, 1.05, false);
  drawMosqueSilhouette(doc, W - 24, 128, 1.05, true);

  // Bordure double dorée
  doc.setDrawColor(...GOLD);
  ...
  if (logoDataUrl) doc.addImage(logoDataUrl, "PNG", W / 2 - 11, 15, 22, 22);
  doc.setTextColor(...EMERALD);
  doc.setFont("times", "bold");
  doc.setFontSize(13);
  doc.text("SEYDA ZEYNAB ACADEMY", W / 2, 43, { align: "center" });
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.3);
  doc.line(W / 2 - 30, 46, W / 2 + 30, 46);

  // Titre en dégradé doré (rendu canvas)
  const titleStyled = await renderStyledTextImage(K.title, {
    fontFamily: "Playfair Display", fontWeight: "800", fontSizePx: 130,
    gradientColors: [[0, "#8a6a24"], [0.5, "#e8c874"], [1, "#8a6a24"]],
  });
  placeStyledText(doc, titleStyled, W / 2, 49, STYLED_TEXT_PX_TO_MM, 170);

  doc.setTextColor(...EMERALD);
  doc.setFont("times", "bold");
  doc.setFontSize(15);
  doc.text(K.subtitle, W / 2, 82, { align: "center" });
  ...
  // Nom de l'élève en calligraphie (rendu canvas, police cursive)
  ...
  placeStyledText(doc, nameStyled, W / 2, 95, STYLED_TEXT_PX_TO_MM, 220);

  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.3);
  doc.line(W / 2 - 34, 120, W / 2 + 34, 120);

  doc.setTextColor(...INK);
  doc.setFont("times", "normal");
  doc.setFontSize(11.5);
  doc.text(K.forLabel, W / 2, 128, { align: "center" });
  ...
  const pillH = 11;
  doc.setFillColor(...EMERALD_DARK);
  doc.roundedRect(W / 2 - pillW / 2, 133, pillW, pillH, 5, 5, "F");
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.5);
  doc.roundedRect(W / 2 - pillW / 2, 133, pillW, pillH, 5, 5, "S");
  doc.setTextColor(255, 255, 255);
  doc.text(opts.achievementTitle, W / 2, 133 + pillH / 2 + 1.6, { align: "center" });
  ...
  const msgLines = doc.splitTextToSize(msg, W - 110);
  doc.text(msgLines, W / 2, 154, { align: "center" });

  doc.setDrawColor(...GOLD_LIGHT);
  doc.setLineWidth(0.3);
  doc.line(W / 2 - 20, 171, W / 2 + 20, 171);
...
async function buildKhatmDiplomaPdf(opts) {
  ...
  drawMosqueSilhouette(doc, 26, 132, 1.25, false);
  drawMosqueSilhouette(doc, W - 26, 132, 1.25, true);

  // Triple bordure dorée premium
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(2.2);
  doc.roundedRect(6, 6, W - 12, H - 12, 6, 6);
  doc.setLineWidth(0.8);
  doc.roundedRect(10, 10, W - 20, H - 20, 9, 9);
  doc.setDrawColor(...GOLD_LIGHT);
  doc.setLineWidth(0.4);
  doc.roundedRect(14, 14, W - 28, H - 28, 12, 12);

  // Motifs de coin (plus grands)
  const cs = 27;
  drawCornerMotif(doc, 20, 20, cs);
  drawCornerMotif(doc, W - 20, 20, cs);
  drawCornerMotif(doc, 20, H - 20, cs);
  drawCornerMotif(doc, W - 20, H - 20, cs);

  const logoDataUrl = await getLogoDataUrl();
  if (logoDataUrl) doc.addImage(logoDataUrl, "PNG", W / 2 - 13, 18, 26, 26);

  doc.setTextColor(...EMERALD);
  doc.setFont("times", "bold");
  doc.setFontSize(13.5);
  doc.text("SEYDA ZEYNAB ACADEMY", W / 2, 50, { align: "center" });
  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.35);
  doc.line(W / 2 - 34, 53, W / 2 + 34, 53);

  const titleStyled = await renderStyledTextImage(L.title, {
    fontFamily: "Playfair Display", fontWeight: "800", fontSizePx: 130,
    gradientColors: [[0, "#7a5c1e"], [0.5, "#f0d68f"], [1, "#7a5c1e"]],
  });
  placeStyledText(doc, titleStyled, W / 2, 57, STYLED_TEXT_PX_TO_MM, 190);

  doc.setTextColor(...EMERALD);
  doc.setFont("times", "bolditalic");
  doc.setFontSize(15);
  doc.text(L.subtitle, W / 2, 94, { align: "center" });

  doc.setTextColor(...INK);
  doc.setFont("times", "italic");
  doc.setFontSize(12);
  doc.text(L.of, W / 2, 104, { align: "center" });

  const nameStyled = await renderStyledTextImage(opts.studentName, {
    fontFamily: "Great Vibes", fontWeight: "400", fontSizePx: 110, color: "#0E6355",
  });
  placeStyledText(doc, nameStyled, W / 2, 107, STYLED_TEXT_PX_TO_MM, 220);

  doc.setDrawColor(...GOLD);
  doc.setLineWidth(0.35);
  doc.line(W / 2 - 40, 134, W / 2 + 40, 134);

  doc.setTextColor(...INK);
  doc.setFont("times", "normal");
  doc.setFontSize(11);
  const bodyLines = doc.splitTextToSize(L.body(opts.studentName), W - 120);
  doc.text(bodyLines, W / 2, 143, { align: "center" });
  ...
  drawSeal(doc, W / 2, footY - 3, 15, ["Khatm", "Al-Qur'an"]);

  const sigX = W - 82;
  doc.setTextColor(...INK);
  doc.setFont("times", "normal");
  doc.setFontSize(9.5);
  doc.text("Seyda Zeynab Academy", sigX, footY - 14, { align: "center" });
  doc.text(L.signature, sigX, footY - 9, { align: "center" });
  const signatureDataUrl = await getSignatureDataUrl();
  if (signatureDataUrl) {
    const sigW = 34;
    const sigImgProps = doc.getImageProperties(signatureDataUrl);
    const sigH = sigW * (sigImgProps.height / sigImgProps.width);
    doc.addImage(signatureDataUrl, "PNG", sigX - sigW / 2, footY - 8, sigW, sigH);
  }
  doc.setDrawColor(...INK);
  doc.setLineWidth(0.2);
  ...
  if (qrDataUrl) {
    const qrSize = 22;
    doc.addImage(qrDataUrl, "PNG", W - 26 - qrSize, footY - 16, qrSize, qrSize);
    doc.setFontSize(7.5);
    doc.setTextColor(140, 125, 90);
    doc.text(L.verify, W - 26 - qrSize / 2, footY + 8, { align: "center" });
  }
