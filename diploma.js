...
async function generateDetailedReport(studentId, fromISO, toISO, lang) {
  ...
  const [quranEntries, topicEntries, surahChecklist] = await Promise.all([
  ...
  const canvas = await buildDetailedReportCanvas({
    ...
    quranItems, checklistItems, athkarItems: byTopic("athkar"), fiqhItems: byTopic("fiqh"), tafsirItems: byTopic("tafsir"), siraItems: byTopic("sira"),
  });
