...
function computeBillingStatus(entity) {
  ...
  // Si un paiement a déjà été enregistré pour un mois ANTÉRIEUR à cette
  // première échéance calculée (ex. élève facturé et payé le jour même de
  // sa création, avant que l'échéance formelle du mois suivant ne soit
  // technique­ment atteinte), on recule le point de départ pour ne pas
  // ignorer ce paiement déjà réel.
  const paymentDates = entity.payments
    .filter((p) => Number(p.amount || 0) > 0 && p.coversMonth)
    .map((p) => new Date(p.coversMonth + "-01T00:00:00"));
  const earliestPayment = paymentDates.length ? new Date(Math.min(...paymentDates)) : null;
  if (earliestPayment && earliestPayment < due) {
    due = new Date(earliestPayment.getFullYear(), earliestPayment.getMonth(), dueDay);
  }
