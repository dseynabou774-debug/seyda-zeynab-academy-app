...
function renderStudentProfile(studentId) {
  ...
  return `
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
      ...
      ${renderProfileJuzChecklist(student, data)}
...
function attachStudentProfileHandlers(studentId) {
  ...
  attachJuzChecklistHandlers(studentId);
...
// -----------------------------------------------------------------------
// 4quater. Checklist "Juz' déjà mémorisés" — même principe que la
// checklist des sourates ci-dessus, mais pour les 30 Juz' du Coran.
// Contrairement à la checklist des sourates (qui a sa propre table dédiée,
// sans historique), celle-ci RÉUTILISE directement le système de
// mémorisation déjà existant (DB.saveJuzProgress / STATE.memorization,
// le même que l'écran #/memorization) : cocher un Juz' ici le marque
// réellement comme mémorisé pour cet élève, ce qui permet de démarrer
// immédiatement un élève avancé dans son parcours de mémorisation sans
// dupliquer les données. Aucun badge ni écran de célébration ne se
// déclenche depuis cette checklist (ce n'est pas "vient de terminer en
// direct", mais "maîtrisait déjà en arrivant") — ces à-côtés restent
// réservés au parcours normal (écran #/memorization, toggleJuzDone).
// -----------------------------------------------------------------------
function renderProfileJuzChecklist(student, data) {
  const memoByJuz = Object.fromEntries((data.memorization || []).map((m) => [m.juzId, m]));
  const doneCount = JUZ_LIST.filter((j) => memoByJuz[j.id]?.completed).length;
  return `
    <div class="card">
      <h3 class="display" style="font-size:16px;margin-bottom:6px">📖 Juz' déjà mémorisés</h3>
      <p class="subject-note" style="font-size:12.5px;margin-bottom:12px">Cochez les Juz' que l'élève a déjà mémorisés avant de rejoindre l'académie (utile pour démarrer un élève avancé directement dans son parcours de mémorisation).</p>
      <div id="spJuzGrid" style="display:flex;flex-wrap:wrap;gap:6px">
        ${JUZ_LIST.map((j) => `
          <button type="button" class="juz-chip" data-juz-id="${j.id}" style="display:flex;align-items:center;gap:5px;font-size:12.5px;padding:6px 10px;border-radius:8px;border:1.5px solid ${memoByJuz[j.id]?.completed ? "var(--color-primary)" : "var(--color-surface-2)"};background:${memoByJuz[j.id]?.completed ? "var(--color-primary)" : "var(--color-surface)"};color:${memoByJuz[j.id]?.completed ? "#fff" : "var(--color-ink)"}">
            <span style="font-family:'Amiri',serif;font-size:14px">${j.name_ar}</span><span>Juz' ${j.id}</span>
          </button>
        `).join("")}
      </div>
      <div id="spJuzCount" style="margin-top:10px;font-size:12px;color:var(--color-ink-soft)">${doneCount}/${JUZ_LIST.length} Juz' mémorisés</div>
    </div>
  `;
}

function attachJuzChecklistHandlers(studentId) {
  document.querySelectorAll("#spJuzGrid .juz-chip").forEach((chip) => {
    chip.addEventListener("click", async () => {
      const juzId = Number(chip.dataset.juzId);
      const memoArr = STATE.studentProfileData.memorization || (STATE.studentProfileData.memorization = []);
      const existing = memoArr.find((m) => m.juzId === juzId);
      const nowCompleted = !existing?.completed;
      const extra = { completed: nowCompleted };
      if (nowCompleted) extra.lastRevisedAt = Date.now();
      const merged = await DB.saveJuzProgress(studentId, juzId, extra);
      const idx = memoArr.findIndex((m) => m.juzId === juzId);
      if (idx >= 0) memoArr[idx] = merged; else memoArr.push(merged);
      chip.style.background = nowCompleted ? "var(--color-primary)" : "var(--color-surface)";
      chip.style.color = nowCompleted ? "#fff" : "var(--color-ink)";
      chip.style.borderColor = nowCompleted ? "var(--color-primary)" : "var(--color-surface-2)";
      const doneCount = memoArr.filter((m) => m.completed).length;
      const counter = document.getElementById("spJuzCount");
      if (counter) counter.textContent = `${doneCount}/${JUZ_LIST.length} Juz' mémorisés`;
    });
  });
}
