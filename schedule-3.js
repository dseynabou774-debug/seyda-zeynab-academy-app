...
// =============================================================================
// CALENDRIER UNIFIÉ — fusionne les créneaux hebdomadaires récurrents avec
// des exceptions ponctuelles (annulation, déplacement, présence) pour une
// date précise, SANS jamais modifier le créneau habituel lui-même, plus
// les cours ponctuels (sans créneau récurrent). Stocké dans "courseLogs".
// =============================================================================

function newAdHocCourseLog({ date, startTime, endTime, title, studentId, studentName, subjects }) {
  return {
    id: newId("clog"), scheduleSlotId: null,
    date, startTime, endTime, title: title || "",
    studentId: studentId || "", studentName: studentName || "",
    subjects: subjects || [], status: "scheduled",
    attendance: null, subjectsCovered: [], remark: "",
    createdAt: Date.now(),
  };
}

// Calcule, pour une date précise, TOUTES les occurrences de cours ce
// jour-là : créneaux récurrents (sauf ceux annulés/déplacés ce jour précis),
// occurrences déplacées VERS ce jour, et cours ponctuels.
function occurrencesForDate(allSlots, allLogs, dateISO) {
  const jsDay = new Date(dateISO + "T00:00:00").getDay();
  const schedDay = (jsDay + 6) % 7; // 0=Lundi ... 6=Dimanche
  const occurrences = [];

  allSlots.filter((s) => !s.paused && s.day === schedDay).forEach((slot) => {
    const override = allLogs.find((l) => l.scheduleSlotId === slot.id && l.date === dateISO);
    if (override && (override.status === "cancelled" || override.status === "moved")) return;
    occurrences.push({
      id: override ? override.id : `virtual_${slot.id}_${dateISO}`,
      scheduleSlotId: slot.id, date: dateISO,
      startTime: slot.startTime, endTime: slot.endTime,
      studentId: slot.studentId, studentName: slot.studentName, subjects: slot.subjects || [],
      studentIds: slot.studentIds && slot.studentIds.length ? slot.studentIds : (slot.studentId ? [slot.studentId] : []),
      title: "", isFamily: !!slot.isFamily,
      status: override?.status || "scheduled",
      attendance: override?.attendance || null,
      subjectsCovered: override?.subjectsCovered || [],
      remark: override?.remark || "",
      isVirtual: !override, isAdHoc: false,
    });
  });

  allLogs.filter((l) => l.scheduleSlotId && l.status === "moved" && l.movedTo?.date === dateISO).forEach((l) => {
    const slot = allSlots.find((s) => s.id === l.scheduleSlotId);
    occurrences.push({
      id: l.id, scheduleSlotId: l.scheduleSlotId, date: dateISO,
      startTime: l.movedTo.startTime || slot?.startTime, endTime: l.movedTo.endTime || slot?.endTime,
      studentId: slot?.studentId || "", studentName: slot?.studentName || "", subjects: slot?.subjects || [],
      studentIds: slot?.studentIds && slot.studentIds.length ? slot.studentIds : (slot?.studentId ? [slot.studentId] : []),
      title: "", isFamily: !!slot?.isFamily,
      status: "moved-in", attendance: l.attendance || null, subjectsCovered: l.subjectsCovered || [], remark: l.remark || "",
      isVirtual: false, isAdHoc: false, originalDate: l.date,
    });
  });

  allLogs.filter((l) => !l.scheduleSlotId && l.date === dateISO && l.status !== "cancelled").forEach((l) => {
    occurrences.push({
      id: l.id, scheduleSlotId: null, date: dateISO,
      startTime: l.startTime, endTime: l.endTime,
      studentId: l.studentId || "", studentName: l.studentName || "", subjects: l.subjects || [],
      studentIds: l.studentId ? [l.studentId] : [],
      title: l.title || "", isFamily: false,
      status: l.status || "scheduled", attendance: l.attendance || null, subjectsCovered: l.subjectsCovered || [], remark: l.remark || "",
      isVirtual: false, isAdHoc: true,
    });
  });

  return occurrences.sort((a, b) => scheduleTimeToMinutes(a.startTime) - scheduleTimeToMinutes(b.startTime));
}

// Sait si une date donnée a au moins une occurrence (pour le point indicateur
// du calendrier mensuel).
function monthOccurrenceDots(allSlots, allLogs, year, month) {
  const daysInMonth = new Date(year, month + 1, 0).getDate();
  const dots = {};
  for (let d = 1; d <= daysInMonth; d++) {
    const iso = `${year}-${String(month + 1).padStart(2, "0")}-${String(d).padStart(2, "0")}`;
    dots[iso] = occurrencesForDate(allSlots, allLogs, iso).length > 0;
  }
  return dots;
}

// -----------------------------------------------------------------------
// Notifications locales. LIMITE HONNÊTE : sans serveur d'envoi (push), un
// navigateur ne peut pas réveiller l'app quand elle est totalement fermée.
// Cette vérification ne fonctionne QUE pendant que l'app est ouverte (au
// premier plan ou récemment en arrière-plan) — ce n'est pas une alarme
// système fiable à 100%, contrairement à une vraie appli native.
// -----------------------------------------------------------------------
...
function checkUpcomingNotifications(allSlots, allLogs) {
  if (typeof Notification === "undefined" || Notification.permission !== "granted") return;
  STATE.notifiedOccurrences = STATE.notifiedOccurrences || new Set();
  const now = new Date();
  const todayStr = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, "0")}-${String(now.getDate()).padStart(2, "0")}`;
  const occs = occurrencesForDate(allSlots, allLogs, todayStr);
  occs.forEach((occ) => {
    if (occ.status === "cancelled" || occ.attendance) return;
    const [h, m] = (occ.startTime || "00:00").split(":").map(Number);
    const startDate = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, m);
    const diffMin = (startDate - now) / 60000;
    const key = `${occ.id}_${occ.date}`;
    if (diffMin > 0 && diffMin <= 15 && !STATE.notifiedOccurrences.has(key)) {
      try {
        new Notification("Seyda Zeynab Academy", {
          body: `${occ.studentName || occ.title || ""} — ${occ.startTime}`,
          icon: "icon-192.png",
        });
      } catch (e) { /* ignore */ }
      STATE.notifiedOccurrences.add(key);
    }
  });
}
