...
// Boot
// -----------------------------------------------------------------------
// NOTE : migrateDiscoverContent() est défini dans discover.js (chargé
// avant app.js). Il ne faut PAS le redéfinir ici : une version en double
// existait auparavant dans ce fichier, avec des valeurs vides par défaut
// au lieu du contenu complet (présentation, matières, tarifs, FAQ...).
// Comme app.js se charge en dernier, cette version vide écrasait
// silencieusement la bonne — d'où une page "Découvrir l'Académie"
// totalement vide pour toute personne ouvrant le lien pour la première
// fois (un parent, par exemple), alors que le téléphone de l'enseignante
// semblait fonctionner normalement (ses propres données remplissaient
// le vide par-dessus). Corrigé le 4 septembre 2026.
...
async function boot() {
  ...
  if (savedSettings) STATE.settings = { language: savedSettings.language, theme: savedSettings.theme, activeStudentId: savedSettings.activeStudentId || null, payment: savedSettings.payment, lastExportedAt: savedSettings.lastExportedAt || null, discover: savedSettings.discover };
  ...
  // IMPORTANT : cette liste doit rester identique à celle utilisée dans
  // route() (isPublicRoute). Un lien public (ex. #/practice-letters,
  // partagé aux élèves pour s'entraîner sans profil) qui manquerait ici
  // serait traité comme une route privée dès le tout premier chargement
  // de la page, et forcerait l'utilisateur vers l'écran de création de
  // profil au lieu d'afficher directement le contenu public attendu.
  const initialPath = hashPath(initialHash);
  const isPublicInitial = initialPath === "#/discover" || initialPath === "#/discover/register" || initialPath.startsWith("#/verify/") || initialPath === "#/practice-letters";
  ...
  if ("serviceWorker" in navigator) {
    ...
    // { updateViaCache: "none" } : empêche le navigateur de réutiliser une
    // copie HTTP en cache du fichier service-worker.js lui-même. Sans ça,
    // le navigateur pouvait continuer à croire — parfois pendant des
    // heures — que l'ancien service worker était toujours le plus récent,
    // ce qui obligeait à vider manuellement le cache pour voir les mises
    // à jour. Avec ce réglage, le navigateur va systématiquement chercher
    // la version la plus fraîche du fichier sur le réseau pour comparer.
    navigator.serviceWorker.register("service-worker.js", { updateViaCache: "none" })
      .then((reg) => {
        // Vérification immédiate au lancement, puis à intervalles réguliers
        // tant que l'app reste ouverte — au lieu d'attendre que le
        // navigateur décide lui-même de vérifier (ce qui peut prendre
        // jusqu'à 24h par défaut). L'enseignante voit ainsi la mise à jour
        // se proposer d'elle-même, sans jamais avoir à vider le cache.
        reg.update().catch(() => {});
...
// Le hash de route peut porter des paramètres après un "?" (ex.
// "#/discover?lang=it&country=IT", utilisé pour que la langue et le pays
// choisis par l'enseignante avant un partage restent identiques une fois
// le lien ouvert sur le téléphone d'un parent). hashPath() retire cette
// partie pour les comparaisons de route ("#/discover" === ...), et
// hashParams() extrait les paramètres eux-mêmes.
function hashPath(hash) { return (hash || "").split("?")[0]; }
...
function route() {
  ...
  if (hash === "#/discover") {
    ...
    if (urlLang && DISCOVER_LANGS.includes(urlLang)) STATE.runtime.discoverLang = urlLang;
    if (urlCountry && DISCOVER_COUNTRIES.includes(urlCountry)) STATE.runtime.discoverCountry = urlCountry;
    return renderPublicShell(renderDiscoverScreen());
  }
...
function renderHome() {
  ...
  return `
    ...
    ${STATE.activeStudentId ? `<button class="btn secondary block" data-go="#/student/${encodeURIComponent(STATE.activeStudentId)}" style="margin-top:14px">🗂️ ${t("openFullProfileBtn")}</button>` : ""}
...
function renderSubjectContent(session, tabKey, subject) {
  ...
  if (tabKey === "coran" && subject.engine === "harakat-combo") {
    return renderHarakatComboMenu(session.id);
  }
  if (tabKey === "coran" && subject.engine === "tanwin-combo") {
    return renderTanwinComboMenu(session.id);
  }
  if (tabKey === "coran" && subject.engine === "longvowels-combo") {
    return renderLongVowelsComboMenu(session.id);
  }
  if (tabKey === "coran" && subject.engine === "sukun-combo") {
    return renderSukunComboMenu(session.id);
  }
  if (tabKey === "coran" && subject.engine === "shadda-combo") {
    return renderShaddaComboMenu(session.id);
  }
...
// Menu Coran de la Séance 2 — mêmes 2 étapes (reconnaissance,
// prononciation) que renderLettersComboMenu, mais sur les 3 Harakat
// (Fatha, Damma, Kasra) au lieu des 28 lettres. subSteps est stocké sous
// la même clé que pour les lettres (subjectProgress(sessionId, "coran")),
// sans risque de collision puisque chaque séance a sa propre progression
// indépendante (sessionId différent).
function renderHarakatComboMenu(sessionId) {
  const done = isSubjectCompleted(sessionId, "coran");
  const p = subjectProgress(sessionId, "coran");
  const subSteps = p.subSteps || {};
  const steps = [
  ...
  return `
    <div class="card subject-card" style="text-align:left">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${steps.map((st) => `
          <button class="path-item" data-harakat-substep="${st.key}" style="background:var(--color-surface-2)">
            <div class="star8 ${st.done ? "done" : "active"}">${st.done ? "✓" : "•"}</div>
            <div class="meta"><div class="title">${st.label}</div></div>
            <div class="chev">›</div>
          </button>
...
// Menu Coran de la Séance 3 — mêmes 2 étapes que renderHarakatComboMenu,
// mais sur les 84 combinaisons lettre+Tanwin.
function renderTanwinComboMenu(sessionId) {
  const done = isSubjectCompleted(sessionId, "coran");
  const p = subjectProgress(sessionId, "coran");
  const subSteps = p.subSteps || {};
  const steps = [
  ...
  return `
    <div class="card subject-card" style="text-align:left">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${steps.map((st) => `
          <button class="path-item" data-tanwin-substep="${st.key}" style="background:var(--color-surface-2)">
            <div class="star8 ${st.done ? "done" : "active"}">${st.done ? "✓" : "•"}</div>
            <div class="meta"><div class="title">${st.label}</div></div>
            <div class="chev">›</div>
          </button>
...
function renderLongVowelsComboMenu(sessionId) {
  const done = isSubjectCompleted(sessionId, "coran");
  const p = subjectProgress(sessionId, "coran");
  const subSteps = p.subSteps || {};
  const steps = [
  ...
  return `
    <div class="card subject-card" style="text-align:left">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${steps.map((st) => `
          <button class="path-item" data-longvowels-substep="${st.key}" style="background:var(--color-surface-2)">
            <div class="star8 ${st.done ? "done" : "active"}">${st.done ? "✓" : "•"}</div>
            <div class="meta"><div class="title">${st.label}</div></div>
            <div class="chev">›</div>
          </button>
...
function renderSukunComboMenu(sessionId) {
  const done = isSubjectCompleted(sessionId, "coran");
  const p = subjectProgress(sessionId, "coran");
  const subSteps = p.subSteps || {};
  const steps = [
  ...
  return `
    <div class="card subject-card" style="text-align:left">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${steps.map((st) => `
          <button class="path-item" data-sukun-substep="${st.key}" style="background:var(--color-surface-2)">
            <div class="star8 ${st.done ? "done" : "active"}">${st.done ? "✓" : "•"}</div>
            <div class="meta"><div class="title">${st.label}</div></div>
            <div class="chev">›</div>
          </button>
...
function renderShaddaComboMenu(sessionId) {
  const done = isSubjectCompleted(sessionId, "coran");
  const p = subjectProgress(sessionId, "coran");
  const subSteps = p.subSteps || {};
  const steps = [
  ...
  return `
    <div class="card subject-card" style="text-align:left">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ${steps.map((st) => `
          <button class="path-item" data-shadda-substep="${st.key}" style="background:var(--color-surface-2)">
            <div class="star8 ${st.done ? "done" : "active"}">${st.done ? "✓" : "•"}</div>
            <div class="meta"><div class="title">${st.label}</div></div>
            <div class="chev">›</div>
          </button>
...
function attachSessionHandlers(sessionId) {
  ...
  document.querySelectorAll("[data-harakat-substep]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const step = btn.dataset.harakatSubstep;
      const root = document.getElementById("subjectContent");
      if (step === "recognition") {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startHarakatFlashcardQuizEngine((percent) => onCoranSubStepDone(sessionId, "recognition", percent));
      } else {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startHarakatListenRepeatEngine((percent) => onCoranSubStepDone(sessionId, "pronunciation", percent), "s2_pron");
      }
    })
  );
  document.querySelectorAll("[data-tanwin-substep]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const step = btn.dataset.tanwinSubstep;
      const root = document.getElementById("subjectContent");
      if (step === "recognition") {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startTanwinFlashcardQuizEngine((percent) => onCoranSubStepDone(sessionId, "recognition", percent));
      } else {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startTanwinListenRepeatEngine((percent) => onCoranSubStepDone(sessionId, "pronunciation", percent), "s3_pron");
      }
    })
  );
  document.querySelectorAll("[data-longvowels-substep]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const step = btn.dataset.longvowelsSubstep;
      const root = document.getElementById("subjectContent");
      if (step === "recognition") {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startLongVowelsFlashcardQuizEngine((percent) => onCoranSubStepDone(sessionId, "recognition", percent));
      } else {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startLongVowelsListenRepeatEngine((percent) => onCoranSubStepDone(sessionId, "pronunciation", percent), "s4_pron");
      }
    })
  );
  document.querySelectorAll("[data-sukun-substep]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const step = btn.dataset.sukunSubstep;
      const root = document.getElementById("subjectContent");
      if (step === "recognition") {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startSukunFlashcardQuizEngine((percent) => onCoranSubStepDone(sessionId, "recognition", percent));
      } else {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startSukunListenRepeatEngine((percent) => onCoranSubStepDone(sessionId, "pronunciation", percent), "s5_pron");
      }
    })
  );
  document.querySelectorAll("[data-shadda-substep]").forEach((btn) =>
    btn.addEventListener("click", () => {
      const step = btn.dataset.shaddaSubstep;
      const root = document.getElementById("subjectContent");
      if (step === "recognition") {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startShaddaFlashcardQuizEngine((percent) => onCoranSubStepDone(sessionId, "recognition", percent));
      } else {
        root.innerHTML = `<div id="engineRoot"></div>`;
        startShaddaListenRepeatEngine((percent) => onCoranSubStepDone(sessionId, "pronunciation", percent), "s6_pron");
      }
    })
  );
}
...
// =============================================================================
// MOTEUR 3 — Harakat (Séance 2) : reconnaissance + écoute/répétition sur
// les 3 voyelles courtes (Fatha, Damma, Kasra). Mêmes principes que les
// moteurs 1 et 2 ci-dessus pour les lettres, adaptés au fait qu'il n'y a
// que 3 éléments au lieu de 28 (donc pas assez d'éléments pour tirer 3
// "distracteurs" différents à chaque question : le quiz propose toujours
// les 3 Harakat comme choix, mélangés, et répète plusieurs fois le tour
// pour obtenir un nombre de questions raisonnable).
// =============================================================================
function startHarakatFlashcardQuizEngine(onComplete) {
  STATE.runtime.hfc = { index: 0, flipped: false, onComplete };
  renderHarakatFlashcard();
}

function renderHarakatFlashcard() {
  const rt = STATE.runtime.hfc;
  const h = HARAKAT[rt.index];
  const root = document.getElementById("engineRoot");
  if (!root) return;
  root.innerHTML = `
    <div class="flash-wrap">
      <div class="flash-counter">${rt.index + 1} / ${HARAKAT.length}</div>
      <div class="flashcard" id="hfcCard">
        ${rt.flipped ? `
          <div class="name">${harakatName(h)}</div>
          <div class="translit">${t("transliteration")} : ${h.translit}</div>
          <div class="hint">${t("flip")}</div>
        ` : `
          <div class="letter arabic">${h.char}</div>
          <div class="hint">${t("flip")}</div>
        `}
      </div>
      <div class="flash-controls">
        <button class="btn secondary" id="hfcPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
        <button class="btn" id="hfcNext">${rt.index === HARAKAT.length - 1 ? t("quizTitle") : t("next")} ›</button>
      </div>
    </div>
  `;
  document.getElementById("hfcCard").addEventListener("click", () => { rt.flipped = !rt.flipped; renderHarakatFlashcard(); });
  document.getElementById("hfcPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); rt.flipped = false; renderHarakatFlashcard(); });
  document.getElementById("hfcNext").addEventListener("click", () => {
    if (rt.index === HARAKAT.length - 1) { startHarakatQuiz(); return; }
    rt.index++; rt.flipped = false; renderHarakatFlashcard();
  });
}

function startHarakatQuiz() {
  // 10 questions tirées parmi les 84 combinaisons (28 lettres × 3 voyelles),
  // avec 3 distracteurs aléatoires par question — même principe que
  // startLetterQuiz, rendu possible maintenant qu'on dispose d'assez
  // d'éléments (contrairement à l'ancienne version à seulement 3 Harakat).
  const questions = shuffle([...HARAKAT]).slice(0, 10).map((h) => {
    const distractors = shuffle(HARAKAT.filter((x) => x.id !== h.id)).slice(0, 3);
    const options = shuffle([h, ...distractors]);
    return { h, options };
  });
  STATE.runtime.hquiz = { questions, index: 0, correct: 0, answered: false, onComplete: STATE.runtime.hfc.onComplete };
  renderHarakatQuizQuestion();
}

function renderHarakatQuizQuestion() {
  const rt = STATE.runtime.hquiz;
  const root = document.getElementById("engineRoot");
  if (rt.index >= rt.questions.length) { rt.onComplete(Math.round((rt.correct / rt.questions.length) * 100)); return; }
  const q = rt.questions[rt.index];
  root.innerHTML = `
    <div class="quiz-question">
      <div class="flash-counter">${t("quizTitle")} — ${rt.index + 1}/${rt.questions.length}</div>
      <div class="prompt-letter arabic">${q.h.char}</div>
      <div style="font-size:13px;color:var(--color-ink-soft)">${t("whichLetterIsThis")}</div>
    </div>
    <div class="quiz-options">
      ${q.options.map((opt) => `<button class="quiz-option" data-id="${opt.id}">${harakatName(opt)}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quizFeedback"></div>
  `;
  root.querySelectorAll(".quiz-option").forEach((btn) =>
    btn.addEventListener("click", () => onHarakatQuizAnswer(btn, q))
  );
}

function onHarakatQuizAnswer(btn, q) {
  const rt = STATE.runtime.hquiz;
  if (rt.answered) return;
  rt.answered = true;
  const chosenId = btn.dataset.id;
  const isCorrect = chosenId === q.h.id;
  if (isCorrect) rt.correct++;
  document.querySelectorAll(".quiz-option").forEach((b) => {
    b.disabled = true;
    if (b.dataset.id === q.h.id) b.classList.add("correct");
    else if (b === btn) b.classList.add("wrong");
  });
  document.getElementById("quizFeedback").textContent = isCorrect ? t("correct") : t("incorrect");
  setTimeout(() => { rt.answered = false; rt.index++; renderHarakatQuizQuestion(); }, 900);
}

function startHarakatListenRepeatEngine(onComplete, recordingNamespace) {
  STATE.runtime.hlr = { index: 0, recorded: {}, mediaRecorder: null, chunks: [], recording: false, onComplete, ns: recordingNamespace };
  renderHarakatListenRepeat();
}

function renderHarakatListenRepeat() {
  const rt = STATE.runtime.hlr;
  const h = HARAKAT[rt.index];
  const root = document.getElementById("engineRoot");
  const hasRecording = !!rt.recorded[h.id];
  root.innerHTML = `
    <div class="flash-counter" style="text-align:center;margin-bottom:6px">${rt.index + 1} / ${HARAKAT.length}</div>
    <div class="card listen-card">
      <div class="listen-letter arabic">${h.char}</div>
      <div class="name">${harakatName(h)}</div>
      <div class="audio-row">
        <button class="btn gold" id="playBtn">🔊 ${t("playAudio")}</button>
      </div>
      <div id="audioMissingNote"></div>
      <audio id="harakatAudio" preload="none">
        ${audioSourcesForHarakat(h.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    ...
    <div class="flash-controls" style="margin-top:18px">
      <button class="btn secondary" id="hlrPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn" id="hlrNext">${rt.index === HARAKAT.length - 1 ? t("finishLevel") : t("next")} ›</button>
    </div>
  `;

  document.getElementById("playBtn").addEventListener("click", () => {
    const audioEl = document.getElementById("harakatAudio");
    document.getElementById("audioMissingNote").innerHTML = "";
    audioEl.play().catch(() => {
      document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
    });
  });
  document.getElementById("harakatAudio").addEventListener("error", () => {
    document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
  });

  document.getElementById("recordBtn").addEventListener("click", () => toggleHarakatRecording(h));
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(STATE.activeStudentId, `${rt.ns}:${h.id}`);
    if (rec?.blob) new Audio(URL.createObjectURL(rec.blob)).play();
  });

  document.getElementById("hlrPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); renderHarakatListenRepeat(); });
  document.getElementById("hlrNext").addEventListener("click", () => {
    if (rt.index === HARAKAT.length - 1) { rt.onComplete(100); return; }
    rt.index++; renderHarakatListenRepeat();
  });
}

async function toggleHarakatRecording(h) {
  const rt = STATE.runtime.hlr;
  if (!rt.recording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      ...
      rt.mediaRecorder.onstop = async () => {
        const blob = new Blob(rt.chunks, { type: "audio/webm" });
        await DB.saveRecording(STATE.activeStudentId, `${rt.ns}:${h.id}`, blob);
        rt.recorded[h.id] = true;
        stream.getTracks().forEach((tr) => tr.stop());
        renderHarakatListenRepeat();
      };
      rt.mediaRecorder.start();
...
// =============================================================================
// MOTEUR 4 — Tanwin (Séance 3) : reconnaissance + écoute/répétition sur
// les 84 combinaisons lettre+Tanwin. Copie conforme des moteurs Harakat
// ci-dessus (mêmes mécanismes), adaptée au tableau TANWIN et à un
// espace de noms d'enregistrement distinct ("s3_pron").
// =============================================================================
function startTanwinFlashcardQuizEngine(onComplete) {
  STATE.runtime.tfc = { index: 0, flipped: false, onComplete };
  renderTanwinFlashcard();
}

function renderTanwinFlashcard() {
  const rt = STATE.runtime.tfc;
  const tw = TANWIN[rt.index];
  const root = document.getElementById("engineRoot");
  if (!root) return;
  root.innerHTML = `
    <div class="flash-wrap">
      <div class="flash-counter">${rt.index + 1} / ${TANWIN.length}</div>
      <div class="flashcard" id="tfcCard">
        ${rt.flipped ? `
          <div class="name">${tanwinName(tw)}</div>
          <div class="translit">${t("transliteration")} : ${tw.translit}</div>
          <div class="hint">${t("flip")}</div>
        ` : `
          <div class="letter arabic">${tw.char}</div>
          <div class="hint">${t("flip")}</div>
        `}
      </div>
      <div class="flash-controls">
        <button class="btn secondary" id="tfcPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
        <button class="btn" id="tfcNext">${rt.index === TANWIN.length - 1 ? t("quizTitle") : t("next")} ›</button>
      </div>
    </div>
  `;
  document.getElementById("tfcCard").addEventListener("click", () => { rt.flipped = !rt.flipped; renderTanwinFlashcard(); });
  document.getElementById("tfcPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); rt.flipped = false; renderTanwinFlashcard(); });
  document.getElementById("tfcNext").addEventListener("click", () => {
    if (rt.index === TANWIN.length - 1) { startTanwinQuiz(); return; }
    rt.index++; rt.flipped = false; renderTanwinFlashcard();
  });
}

function startTanwinQuiz() {
  const questions = shuffle([...TANWIN]).slice(0, 10).map((tw) => {
    const distractors = shuffle(TANWIN.filter((x) => x.id !== tw.id)).slice(0, 3);
    const options = shuffle([tw, ...distractors]);
    return { tw, options };
  });
  STATE.runtime.tquiz = { questions, index: 0, correct: 0, answered: false, onComplete: STATE.runtime.tfc.onComplete };
  renderTanwinQuizQuestion();
}

function renderTanwinQuizQuestion() {
  const rt = STATE.runtime.tquiz;
  const root = document.getElementById("engineRoot");
  if (rt.index >= rt.questions.length) { rt.onComplete(Math.round((rt.correct / rt.questions.length) * 100)); return; }
  const q = rt.questions[rt.index];
  root.innerHTML = `
    <div class="quiz-question">
      <div class="flash-counter">${t("quizTitle")} — ${rt.index + 1}/${rt.questions.length}</div>
      <div class="prompt-letter arabic">${q.tw.char}</div>
      <div style="font-size:13px;color:var(--color-ink-soft)">${t("whichLetterIsThis")}</div>
    </div>
    <div class="quiz-options">
      ${q.options.map((opt) => `<button class="quiz-option" data-id="${opt.id}">${tanwinName(opt)}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quizFeedback"></div>
  `;
  root.querySelectorAll(".quiz-option").forEach((btn) =>
    btn.addEventListener("click", () => onTanwinQuizAnswer(btn, q))
  );
}

function onTanwinQuizAnswer(btn, q) {
  const rt = STATE.runtime.tquiz;
  if (rt.answered) return;
  rt.answered = true;
  const chosenId = btn.dataset.id;
  const isCorrect = chosenId === q.tw.id;
  if (isCorrect) rt.correct++;
  document.querySelectorAll(".quiz-option").forEach((b) => {
    b.disabled = true;
    if (b.dataset.id === q.tw.id) b.classList.add("correct");
    else if (b === btn) b.classList.add("wrong");
  });
  document.getElementById("quizFeedback").textContent = isCorrect ? t("correct") : t("incorrect");
  setTimeout(() => { rt.answered = false; rt.index++; renderTanwinQuizQuestion(); }, 900);
}

function startTanwinListenRepeatEngine(onComplete, recordingNamespace) {
  STATE.runtime.tlr = { index: 0, recorded: {}, mediaRecorder: null, chunks: [], recording: false, onComplete, ns: recordingNamespace };
  renderTanwinListenRepeat();
}

function renderTanwinListenRepeat() {
  const rt = STATE.runtime.tlr;
  const tw = TANWIN[rt.index];
  const root = document.getElementById("engineRoot");
  const hasRecording = !!rt.recorded[tw.id];
  root.innerHTML = `
    <div class="flash-counter" style="text-align:center;margin-bottom:6px">${rt.index + 1} / ${TANWIN.length}</div>
    <div class="card listen-card">
      <div class="listen-letter arabic">${tw.char}</div>
      <div class="name">${tanwinName(tw)}</div>
      <div class="audio-row">
        <button class="btn gold" id="playBtn">🔊 ${t("playAudio")}</button>
      </div>
      <div id="audioMissingNote"></div>
      <audio id="tanwinAudio" preload="none">
        ${audioSourcesForTanwin(tw.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    ...
    <div class="flash-controls" style="margin-top:18px">
      <button class="btn secondary" id="tlrPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn" id="tlrNext">${rt.index === TANWIN.length - 1 ? t("finishLevel") : t("next")} ›</button>
    </div>
  `;

  document.getElementById("playBtn").addEventListener("click", () => {
    const audioEl = document.getElementById("tanwinAudio");
    document.getElementById("audioMissingNote").innerHTML = "";
    audioEl.play().catch(() => {
      document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
    });
  });
  document.getElementById("tanwinAudio").addEventListener("error", () => {
    document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
  });

  document.getElementById("recordBtn").addEventListener("click", () => toggleTanwinRecording(tw));
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(STATE.activeStudentId, `${rt.ns}:${tw.id}`);
    if (rec?.blob) new Audio(URL.createObjectURL(rec.blob)).play();
  });

  document.getElementById("tlrPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); renderTanwinListenRepeat(); });
  document.getElementById("tlrNext").addEventListener("click", () => {
    if (rt.index === TANWIN.length - 1) { rt.onComplete(100); return; }
    rt.index++; renderTanwinListenRepeat();
  });
}

async function toggleTanwinRecording(tw) {
  const rt = STATE.runtime.tlr;
  if (!rt.recording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      ...
      rt.mediaRecorder.onstop = async () => {
        const blob = new Blob(rt.chunks, { type: "audio/webm" });
        await DB.saveRecording(STATE.activeStudentId, `${rt.ns}:${tw.id}`, blob);
        rt.recorded[tw.id] = true;
        stream.getTracks().forEach((tr) => tr.stop());
        renderTanwinListenRepeat();
      };
      rt.mediaRecorder.start();
...
// =============================================================================
// MOTEUR — Séance 4 (Voyelles longues) : reconnaissance + écoute/répétition, copie
// conforme des moteurs Harakat/Tanwin ci-dessus adaptée au tableau
// LONG_VOWELS.
// =============================================================================
function startLongVowelsFlashcardQuizEngine(onComplete) {
  STATE.runtime.lvfc = { index: 0, flipped: false, onComplete };
  renderLongVowelsFlashcard();
}

function renderLongVowelsFlashcard() {
  const rt = STATE.runtime.lvfc;
  const item = LONG_VOWELS[rt.index];
  const root = document.getElementById("engineRoot");
  if (!root) return;
  root.innerHTML = `
    <div class="flash-wrap">
      <div class="flash-counter">${rt.index + 1} / ${LONG_VOWELS.length}</div>
      <div class="flashcard" id="lvfcCard">
        ${rt.flipped ? `
          <div class="name">${longVowelName(item)}</div>
          <div class="translit">${t("transliteration")} : ${item.translit}</div>
          <div class="hint">${t("flip")}</div>
        ` : `
      ...
      <div class="flash-controls">
        <button class="btn secondary" id="lvfcPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
        <button class="btn" id="lvfcNext">${rt.index === LONG_VOWELS.length - 1 ? t("quizTitle") : t("next")} ›</button>
      </div>
    </div>
  `;
  document.getElementById("lvfcCard").addEventListener("click", () => { rt.flipped = !rt.flipped; renderLongVowelsFlashcard(); });
  document.getElementById("lvfcPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); rt.flipped = false; renderLongVowelsFlashcard(); });
  document.getElementById("lvfcNext").addEventListener("click", () => {
    if (rt.index === LONG_VOWELS.length - 1) { startLongVowelsQuiz(); return; }
    rt.index++; rt.flipped = false; renderLongVowelsFlashcard();
  });
}

function startLongVowelsQuiz() {
  const questions = shuffle([...LONG_VOWELS]).slice(0, 10).map((item) => {
    const distractors = shuffle(LONG_VOWELS.filter((x) => x.id !== item.id)).slice(0, 3);
    const options = shuffle([item, ...distractors]);
    return { item, options };
  });
  STATE.runtime.lvquiz = { questions, index: 0, correct: 0, answered: false, onComplete: STATE.runtime.lvfc.onComplete };
  renderLongVowelsQuizQuestion();
}

function renderLongVowelsQuizQuestion() {
  const rt = STATE.runtime.lvquiz;
  const root = document.getElementById("engineRoot");
  if (rt.index >= rt.questions.length) { rt.onComplete(Math.round((rt.correct / rt.questions.length) * 100)); return; }
  const q = rt.questions[rt.index];
  root.innerHTML = `
    <div class="quiz-question">
      <div class="flash-counter">${t("quizTitle")} — ${rt.index + 1}/${rt.questions.length}</div>
      <div class="prompt-letter arabic">${q.item.char}</div>
      <div style="font-size:13px;color:var(--color-ink-soft)">${t("whichLetterIsThis")}</div>
    </div>
    <div class="quiz-options">
      ${q.options.map((opt) => `<button class="quiz-option" data-id="${opt.id}">${longVowelName(opt)}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quizFeedback"></div>
  `;
  root.querySelectorAll(".quiz-option").forEach((btn) =>
    btn.addEventListener("click", () => onLongVowelsQuizAnswer(btn, q))
  );
}

function onLongVowelsQuizAnswer(btn, q) {
  const rt = STATE.runtime.lvquiz;
  if (rt.answered) return;
  rt.answered = true;
  const chosenId = btn.dataset.id;
  const isCorrect = chosenId === q.item.id;
  if (isCorrect) rt.correct++;
  document.querySelectorAll(".quiz-option").forEach((b) => {
    b.disabled = true;
  ...
  document.getElementById("quizFeedback").textContent = isCorrect ? t("correct") : t("incorrect");
  setTimeout(() => { rt.answered = false; rt.index++; renderLongVowelsQuizQuestion(); }, 900);
}

function startLongVowelsListenRepeatEngine(onComplete, recordingNamespace) {
  STATE.runtime.lvlr = { index: 0, recorded: {}, mediaRecorder: null, chunks: [], recording: false, onComplete, ns: recordingNamespace };
  renderLongVowelsListenRepeat();
}

function renderLongVowelsListenRepeat() {
  const rt = STATE.runtime.lvlr;
  const item = LONG_VOWELS[rt.index];
  const root = document.getElementById("engineRoot");
  const hasRecording = !!rt.recorded[item.id];
  root.innerHTML = `
    <div class="flash-counter" style="text-align:center;margin-bottom:6px">${rt.index + 1} / ${LONG_VOWELS.length}</div>
    <div class="card listen-card">
      <div class="listen-letter arabic">${item.char}</div>
      <div class="name">${longVowelName(item)}</div>
      <div class="audio-row">
        <button class="btn gold" id="playBtn">🔊 ${t("playAudio")}</button>
      </div>
      <div id="audioMissingNote"></div>
      <audio id="lvAudio" preload="none">
        ${audioSourcesForLongVowels(item.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    ...
    <div class="flash-controls" style="margin-top:18px">
      <button class="btn secondary" id="lvlrPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn" id="lvlrNext">${rt.index === LONG_VOWELS.length - 1 ? t("finishLevel") : t("next")} ›</button>
    </div>
  `;

  document.getElementById("playBtn").addEventListener("click", () => {
    const audioEl = document.getElementById("lvAudio");
    document.getElementById("audioMissingNote").innerHTML = "";
    audioEl.play().catch(() => {
      document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
    });
  });
  document.getElementById("lvAudio").addEventListener("error", () => {
    document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
  });

  document.getElementById("recordBtn").addEventListener("click", () => toggleLongVowelsRecording(item));
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(STATE.activeStudentId, `${rt.ns}:${item.id}`);
    if (rec?.blob) new Audio(URL.createObjectURL(rec.blob)).play();
  });

  document.getElementById("lvlrPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); renderLongVowelsListenRepeat(); });
  document.getElementById("lvlrNext").addEventListener("click", () => {
    if (rt.index === LONG_VOWELS.length - 1) { rt.onComplete(100); return; }
    rt.index++; renderLongVowelsListenRepeat();
  });
}

async function toggleLongVowelsRecording(item) {
  const rt = STATE.runtime.lvlr;
  if (!rt.recording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
      ...
      rt.mediaRecorder.onstop = async () => {
        const blob = new Blob(rt.chunks, { type: "audio/webm" });
        await DB.saveRecording(STATE.activeStudentId, `${rt.ns}:${item.id}`, blob);
        rt.recorded[item.id] = true;
        stream.getTracks().forEach((tr) => tr.stop());
        renderLongVowelsListenRepeat();
      };
      rt.mediaRecorder.start();
...
// =============================================================================
// MOTEUR — Séance 5 (Soukoun) : reconnaissance + écoute/répétition, copie
// conforme des moteurs Harakat/Tanwin ci-dessus adaptée au tableau
// SUKUN.
// =============================================================================
function startSukunFlashcardQuizEngine(onComplete) {
  STATE.runtime.sufc = { index: 0, flipped: false, onComplete };
  renderSukunFlashcard();
}

function renderSukunFlashcard() {
  const rt = STATE.runtime.sufc;
  const item = SUKUN[rt.index];
  const root = document.getElementById("engineRoot");
  if (!root) return;
  root.innerHTML = `
    <div class="flash-wrap">
      <div class="flash-counter">${rt.index + 1} / ${SUKUN.length}</div>
      <div class="flashcard" id="sufcCard">
        ${rt.flipped ? `
          <div class="name">${sukunName(item)}</div>
          <div class="translit">${t("transliteration")} : ${item.translit}</div>
          <div class="hint">${t("flip")}</div>
        ` : `
          <div class="letter arabic">${item.char}</div>
      ...
      <div class="flash-controls">
        <button class="btn secondary" id="sufcPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
        <button class="btn" id="sufcNext">${rt.index === SUKUN.length - 1 ? t("quizTitle") : t("next")} ›</button>
      </div>
    </div>
  `;
  document.getElementById("sufcCard").addEventListener("click", () => { rt.flipped = !rt.flipped; renderSukunFlashcard(); });
  document.getElementById("sufcPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); rt.flipped = false; renderSukunFlashcard(); });
  document.getElementById("sufcNext").addEventListener("click", () => {
    if (rt.index === SUKUN.length - 1) { startSukunQuiz(); return; }
    rt.index++; rt.flipped = false; renderSukunFlashcard();
  });
}

function startSukunQuiz() {
  const questions = shuffle([...SUKUN]).slice(0, 10).map((item) => {
    const distractors = shuffle(SUKUN.filter((x) => x.id !== item.id)).slice(0, 3);
    const options = shuffle([item, ...distractors]);
    return { item, options };
  });
  STATE.runtime.suquiz = { questions, index: 0, correct: 0, answered: false, onComplete: STATE.runtime.sufc.onComplete };
  renderSukunQuizQuestion();
}

function renderSukunQuizQuestion() {
  const rt = STATE.runtime.suquiz;
  const root = document.getElementById("engineRoot");
  if (rt.index >= rt.questions.length) { rt.onComplete(Math.round((rt.correct / rt.questions.length) * 100)); return; }
  const q = rt.questions[rt.index];
  root.innerHTML = `
    ...
    <div class="quiz-options">
      ${q.options.map((opt) => `<button class="quiz-option" data-id="${opt.id}">${sukunName(opt)}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quizFeedback"></div>
  `;
  root.querySelectorAll(".quiz-option").forEach((btn) =>
    btn.addEventListener("click", () => onSukunQuizAnswer(btn, q))
  );
}

function onSukunQuizAnswer(btn, q) {
  const rt = STATE.runtime.suquiz;
  if (rt.answered) return;
  rt.answered = true;
  const chosenId = btn.dataset.id;
  ...
  document.getElementById("quizFeedback").textContent = isCorrect ? t("correct") : t("incorrect");
  setTimeout(() => { rt.answered = false; rt.index++; renderSukunQuizQuestion(); }, 900);
}

function startSukunListenRepeatEngine(onComplete, recordingNamespace) {
  STATE.runtime.sulr = { index: 0, recorded: {}, mediaRecorder: null, chunks: [], recording: false, onComplete, ns: recordingNamespace };
  renderSukunListenRepeat();
}

function renderSukunListenRepeat() {
  const rt = STATE.runtime.sulr;
  const item = SUKUN[rt.index];
  const root = document.getElementById("engineRoot");
  const hasRecording = !!rt.recorded[item.id];
  root.innerHTML = `
    <div class="flash-counter" style="text-align:center;margin-bottom:6px">${rt.index + 1} / ${SUKUN.length}</div>
    <div class="card listen-card">
      <div class="listen-letter arabic">${item.char}</div>
      <div class="name">${sukunName(item)}</div>
      <div class="audio-row">
      ...
      <div id="audioMissingNote"></div>
      <audio id="suAudio" preload="none">
        ${audioSourcesForSukun(item.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    ...
    <div class="flash-controls" style="margin-top:18px">
      <button class="btn secondary" id="sulrPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn" id="sulrNext">${rt.index === SUKUN.length - 1 ? t("finishLevel") : t("next")} ›</button>
    </div>
  `;

  document.getElementById("playBtn").addEventListener("click", () => {
    const audioEl = document.getElementById("suAudio");
    document.getElementById("audioMissingNote").innerHTML = "";
    audioEl.play().catch(() => {
      document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
    });
  });
  document.getElementById("suAudio").addEventListener("error", () => {
    document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
  });

  document.getElementById("recordBtn").addEventListener("click", () => toggleSukunRecording(item));
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(STATE.activeStudentId, `${rt.ns}:${item.id}`);
    if (rec?.blob) new Audio(URL.createObjectURL(rec.blob)).play();
  });

  document.getElementById("sulrPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); renderSukunListenRepeat(); });
  document.getElementById("sulrNext").addEventListener("click", () => {
    if (rt.index === SUKUN.length - 1) { rt.onComplete(100); return; }
    rt.index++; renderSukunListenRepeat();
  });
}

async function toggleSukunRecording(item) {
  const rt = STATE.runtime.sulr;
  if (!rt.recording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
...
// =============================================================================
// MOTEUR — Séance 6 (Chadda) : reconnaissance + écoute/répétition, copie
// conforme des moteurs Harakat/Tanwin ci-dessus adaptée au tableau
// SHADDA.
// =============================================================================
function startShaddaFlashcardQuizEngine(onComplete) {
  STATE.runtime.shfc = { index: 0, flipped: false, onComplete };
  renderShaddaFlashcard();
}

function renderShaddaFlashcard() {
  const rt = STATE.runtime.shfc;
  const item = SHADDA[rt.index];
  const root = document.getElementById("engineRoot");
  if (!root) return;
  root.innerHTML = `
    <div class="flash-wrap">
      <div class="flash-counter">${rt.index + 1} / ${SHADDA.length}</div>
      <div class="flashcard" id="shfcCard">
        ${rt.flipped ? `
          <div class="name">${shaddaName(item)}</div>
          <div class="translit">${t("transliteration")} : ${item.translit}</div>
          <div class="hint">${t("flip")}</div>
        ` : `
          <div class="letter arabic">${item.char}</div>
      ...
      <div class="flash-controls">
        <button class="btn secondary" id="shfcPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
        <button class="btn" id="shfcNext">${rt.index === SHADDA.length - 1 ? t("quizTitle") : t("next")} ›</button>
      </div>
    </div>
  `;
  document.getElementById("shfcCard").addEventListener("click", () => { rt.flipped = !rt.flipped; renderShaddaFlashcard(); });
  document.getElementById("shfcPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); rt.flipped = false; renderShaddaFlashcard(); });
  document.getElementById("shfcNext").addEventListener("click", () => {
    if (rt.index === SHADDA.length - 1) { startShaddaQuiz(); return; }
    rt.index++; rt.flipped = false; renderShaddaFlashcard();
  });
}

function startShaddaQuiz() {
  const questions = shuffle([...SHADDA]).slice(0, 10).map((item) => {
    const distractors = shuffle(SHADDA.filter((x) => x.id !== item.id)).slice(0, 3);
    const options = shuffle([item, ...distractors]);
    return { item, options };
  });
  STATE.runtime.shquiz = { questions, index: 0, correct: 0, answered: false, onComplete: STATE.runtime.shfc.onComplete };
  renderShaddaQuizQuestion();
}

function renderShaddaQuizQuestion() {
  const rt = STATE.runtime.shquiz;
  const root = document.getElementById("engineRoot");
  if (rt.index >= rt.questions.length) { rt.onComplete(Math.round((rt.correct / rt.questions.length) * 100)); return; }
  const q = rt.questions[rt.index];
  root.innerHTML = `
    ...
    <div class="quiz-options">
      ${q.options.map((opt) => `<button class="quiz-option" data-id="${opt.id}">${shaddaName(opt)}</button>`).join("")}
    </div>
    <div class="quiz-feedback" id="quizFeedback"></div>
  `;
  root.querySelectorAll(".quiz-option").forEach((btn) =>
    btn.addEventListener("click", () => onShaddaQuizAnswer(btn, q))
  );
}

function onShaddaQuizAnswer(btn, q) {
  const rt = STATE.runtime.shquiz;
  if (rt.answered) return;
  rt.answered = true;
  const chosenId = btn.dataset.id;
  ...
  document.getElementById("quizFeedback").textContent = isCorrect ? t("correct") : t("incorrect");
  setTimeout(() => { rt.answered = false; rt.index++; renderShaddaQuizQuestion(); }, 900);
}

function startShaddaListenRepeatEngine(onComplete, recordingNamespace) {
  STATE.runtime.shlr = { index: 0, recorded: {}, mediaRecorder: null, chunks: [], recording: false, onComplete, ns: recordingNamespace };
  renderShaddaListenRepeat();
}

function renderShaddaListenRepeat() {
  const rt = STATE.runtime.shlr;
  const item = SHADDA[rt.index];
  const root = document.getElementById("engineRoot");
  const hasRecording = !!rt.recorded[item.id];
  root.innerHTML = `
    <div class="flash-counter" style="text-align:center;margin-bottom:6px">${rt.index + 1} / ${SHADDA.length}</div>
    <div class="card listen-card">
      <div class="listen-letter arabic">${item.char}</div>
      <div class="name">${shaddaName(item)}</div>
      <div class="audio-row">
      ...
      <div id="audioMissingNote"></div>
      <audio id="shAudio" preload="none">
        ${audioSourcesForShadda(item.id).map((src) => `<source src="${src}">`).join("")}
      </audio>
    ...
    <div class="flash-controls" style="margin-top:18px">
      <button class="btn secondary" id="shlrPrev" ${rt.index === 0 ? "disabled" : ""}>‹ ${t("previous")}</button>
      <button class="btn" id="shlrNext">${rt.index === SHADDA.length - 1 ? t("finishLevel") : t("next")} ›</button>
    </div>
  `;

  document.getElementById("playBtn").addEventListener("click", () => {
    const audioEl = document.getElementById("shAudio");
    document.getElementById("audioMissingNote").innerHTML = "";
    audioEl.play().catch(() => {
      document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
    });
  });
  document.getElementById("shAudio").addEventListener("error", () => {
    document.getElementById("audioMissingNote").innerHTML = `<div class="audio-missing-note">${t("audioMissing")}</div>`;
  });

  document.getElementById("recordBtn").addEventListener("click", () => toggleShaddaRecording(item));
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(STATE.activeStudentId, `${rt.ns}:${item.id}`);
    if (rec?.blob) new Audio(URL.createObjectURL(rec.blob)).play();
  });

  document.getElementById("shlrPrev").addEventListener("click", () => { rt.index = Math.max(0, rt.index - 1); renderShaddaListenRepeat(); });
  document.getElementById("shlrNext").addEventListener("click", () => {
    if (rt.index === SHADDA.length - 1) { rt.onComplete(100); return; }
    rt.index++; renderShaddaListenRepeat();
  });
}

async function toggleShaddaRecording(item) {
  const rt = STATE.runtime.shlr;
  if (!rt.recording) {
    try {
      const stream = await navigator.mediaDevices.getUserMedia({ audio: true });
...
function renderProgress() {
  ...
  return `
    ...
    ${STATE.activeStudentId ? `<button class="btn secondary block" data-go="#/student/${encodeURIComponent(STATE.activeStudentId)}" style="margin-bottom:14px">🗂️ ${t("openFullProfileBtn")}</button>` : ""}
  ...
  return `
    ...
    <div style="display:flex;flex-direction:column;gap:10px">
      ...
      <label class="btn secondary block" style="text-align:center;cursor:pointer">
        ⬆ ${t("importData")}
        <input type="file" id="importInput" accept="application/json,.json,text/plain,*/*" style="display:none" />
      </label>
    </div>
...
function attachSettingsHandlers() {
  ...
  document.getElementById("importInput").addEventListener("change", async (e) => {
    ...
    if (!file) return;
    toast("Importation en cours, patientez...");
    ...
    try {
      ...
      toast("OK");
      location.reload();
    } catch (err) { console.error("Erreur d'import", err); toast("Erreur / Error"); }
  });
}
...
function attachPaymentDetailHandlers(entityId) {
  ...
  document.getElementById("markUpToDateBtn")?.addEventListener("click", () => {
    openConfirmModal(t("markUpToDateTitle"), t("markUpToDateMsg"), async () => {
      // Avant correction, ce bouton se contentait de réinitialiser
      // entity.startDate à aujourd'hui, sans jamais enregistrer de
      // paiement — le calcul de statut (computeBillingStatus) ne peut
      // jamais devenir "payé" sans un paiement réel couvrant la période
      // en cours, donc l'élève restait affiché "En attente" indéfiniment.
      // On enregistre maintenant un vrai paiement pour la période
      // actuellement due, du montant plein, ce qui fait passer le statut
      // à "À jour" comme attendu.
      const st = computeBillingStatus(entity);
      const periodKey = st.currentCycle ? st.currentCycle.periodKey : periodKeyOf(firstDueDate(entity.startDate, entity.dueDay || 1));
      recordPayment(entity, entity.refId, {
        amount: entity.amount,
        currency: entity.currency,
        date: todayISO(),
        coversMonth: periodKey,
        method: STATE.settings.payment.paymentMethods?.[0] || "",
        comment: t("markUpToDateToast"),
      });
...
function renderDiscoverScreen() {
  ...
  const isRtl = lang === "ar";
  // Repli en cascade : pays sélectionné -> Sénégal -> premier pays
  // disponible -> objet vide sûr. Empêche tout plantage si un pays a été
  // renommé/supprimé via "Modifier les tarifs par pays".
  const countryData = c.countries[country] || c.countries.SN || Object.values(c.countries || {})[0] || { label: "", currency: "", plans: [] };
...
function attachDiscoverScreenHandlers() {
  ...
  // Le lien partagé embarque la langue ET le pays actuellement affichés
  // (onglets sélectionnés par l'enseignante), pour que le parent qui
  // l'ouvre voie exactement le même contenu, quelle que soit la langue
  // réglée sur son propre téléphone.
  const shareLang = STATE.runtime.discoverLang || "fr";
  const shareCountry = STATE.runtime.discoverCountry || "SN";
  const shareUrl = location.origin + location.pathname + `#/discover?lang=${shareLang}&country=${shareCountry}`;
