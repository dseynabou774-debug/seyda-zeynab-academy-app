...
function openBulletinModal() {
  ...
  openModal(`
    ...
    <div class="field" style="margin:14px 0 18px">
      <label>${t("teacherComment")}</label>
      <textarea id="bulletinComment" rows="4" placeholder="${t("teacherCommentPlaceholder")}"
  ...
  document.getElementById("confirmBulletinBtn").addEventListener("click", async () => {
    ...
    const { dataUrl, filename, studentName } = await generateBulletin(type, sessionId, chosenLang, comment);
    closeModal();
