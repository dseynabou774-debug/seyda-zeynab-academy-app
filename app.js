  ...
  const rows = filtered
    .map(({ e, st }) => `
      <button class="path-item" data-entity="${e.id}" style="background:var(--color-surface);box-shadow:var(--shadow-card)">
        ...
        <div class="meta">
          <div class="title">${e.name}${e.country ? ` <span style="font-size:11px;color:var(--color-ink-soft)">· ${STATE.settings.discover.countries[e.country]?.label || e.country}</span>` : ""}</div>
