...
const STATE = {
  ...
  progress: {},   // { [levelId]: {completed, percent, stars} }
...
async function boot() {
  ...
  if (savedSettings) STATE.settings = { language: savedSettings.language, theme: savedSettings.theme };
  applyTheme();

  STATE.student = await DB.getStudent();
  STATE.progress = Object.fromEntries((await DB.getAllProgress()).map((p) => [p.levelId, p]));
  STATE.badges = await DB.getAllBadges();
  STATE.diplomas = await DB.getAllDiplomas();
...
function renderOnboarding() {
  ...
  document.getElementById("createProfileBtn").addEventListener("click", async () => {
    ...
    STATE.student = await DB.saveStudent(fullName, ageGroup);
    go("#/home");
  });
}
...
function renderListenRepeat(level) {
  ...
  document.getElementById("playRecBtn")?.addEventListener("click", async () => {
    const rec = await DB.getRecording(`${level.id}:${letter.id}`);
  ...
  if (!rt.recording) {
    try {
      ...
      rt.mediaRecorder.onstop = async () => {
        ...
        await DB.saveRecording(`${level.id}:${letter.id}`, blob);
...
async function finishLevel(level, percent) {
  await DB.saveProgress(level.id, { completed: true, percent, stars: percent >= 80 ? 3 : percent >= 50 ? 2 : 1 });
...
function renderSettings() {
  return `
    <div class="section-title">${t("settings_student")}</div>
    <div class="card"><b>${STATE.student?.fullName || ""}</b></div>
