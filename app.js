...
function renderSubjectContent(session, tabKey, subject) {
  ...
  if (subject.engine === "content-checklist") {
    const title = subjectTitle(subject);
    return `
      <div class="card subject-card">
        <div class="subject-icon">${SUBJECT_TABS.find((t2) => t2.key === tabKey).icon}</div>
        ${title ? `<h3 class="display">${title}</h3>` : `<h3 class="display" style="color:var(--color-ink-soft)">${t("toDefine")}</h3>`}
        <p class="subject-note">${title ? t("subjectLiveNote") : t("subjectToDefineNote")}</p>
        <button class="btn ${done ? "secondary" : "gold"} block" id="markDoneBtn" ${!title ? "disabled" : ""}>
  ...
  // engine null : contenu Coran pas encore construit (séances 2+)
  return `
    <div class="card subject-card">
      <div class="subject-icon">📖</div>
      <h3 class="display">${subjectTitle(subject) || t("toDefine")}</h3>
      <p class="subject-note">${t("comingSoon")}</p>
    </div>
  `;
}
...
function attachSessionHandlers(sessionId) {
  ...
  document.getElementById("markDoneBtn")?.addEventListener("click", async () => {
    const activeTab = STATE.runtime.session.activeTab;
    await finishSubject(sessionId, activeTab);
  });
...
async function onCoranSubStepDone(sessionId, stepKey, percent) {
  ...
  if (bothDone) {
    await onSubjectFullyDone(sessionId, "coran");
  } else {
...
async function finishSubject(sessionId, subjectKey) {
  await markSubjectProgress(sessionId, subjectKey, { completed: true });
  await onSubjectFullyDone(sessionId, subjectKey);
}

async function onSubjectFullyDone(sessionId, subjectKey) {
