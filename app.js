...
async function boot() {
  ...
  STATE.settings.discover = { ...defaultDiscoverContent(), ...(STATE.settings.discover || {}) };
  applyTheme();
...
function route() {
  ...
  if (verifyMatch) return renderPublicShell(renderVerifyScreen(decodeURIComponent(verifyMatch[1])));
...
function openEditDiscoverModal() {
  ...
  openModal(`
    <div class="close-row"><button class="icon-btn" onclick="closeModal()">✕</button></div>
    <h3 class="display" style="text-align:center;margin-bottom:14px">${t("editDiscoverContent")}</h3>

    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionAbout")}</label>
      <textarea id="dDescription" rows="3" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.description}</textarea>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionSubjects")} (${t("oneLinePerItem")})</label>
      <textarea id="dSubjects" rows="4" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.subjects.join("\n")}</textarea>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionFlow")}</label>
      <textarea id="dFlow" rows="3" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.courseFlow}</textarea>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionPlans")} (${t("formatPlans")})</label>
      <textarea id="dPlans" rows="3" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.plans.map((p) => `${p.name} | ${p.amount} | ${p.currency} | ${p.frequency} | ${p.description}`).join("\n")}</textarea>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionSchedule")} (${t("oneLinePerItem")})</label>
      <textarea id="dSlots" rows="3" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.scheduleSlots.map((s) => s.label).join("\n")}</textarea>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("sectionRules")} (${t("oneLinePerItem")})</label>
      <textarea id="dRules" rows="4" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.rules.join("\n")}</textarea>
    </div>
    <div class="field" style="margin-bottom:18px">
      <label>${dt("sectionFaq")} (${t("formatFaq")})</label>
      <textarea id="dFaq" rows="4" style="padding:12px;border-radius:10px;border:1px solid var(--color-surface-2);background:var(--color-surface);color:var(--color-ink);font-size:14px;font-family:inherit;resize:vertical">${c.faq.map((f) => `${f.q} | ${f.a}`).join("\n")}</textarea>
    </div>
  ...
  document.getElementById("confirmDiscoverEditBtn").addEventListener("click", async () => {
    ...
    STATE.settings.discover = {
      ...
      subjects: linesOf("dSubjects"),
      courseFlow: document.getElementById("dFlow").value.trim(),
      plans: linesOf("dPlans").map((line) => {
        const [name, amount, currency, frequency, description] = line.split("|").map((s) => s.trim());
        return { name: name || "", amount: amount || "", currency: currency || "", frequency: frequency || "", description: description || "" };
      }),
      scheduleSlots: linesOf("dSlots").map((label) => ({ label, available: true })),
      rules: linesOf("dRules"),
      faq: linesOf("dFaq").map((line) => {
...
// reçu, d'un diplôme ou d'un bulletin (lien #/verify/...). L'app étant
// 100% locale (aucun serveur), les informations affichées proviennent
// directement du contenu encodé dans le lien/QR lui-même, pas d'une base
// de données distante : la page confirme donc que le document scanné
// correspond bien aux informations qu'il affiche, sans dépendre d'une
// connexion à l'appareil de l'enseignante.
// =============================================================================
function renderVerifyScreen(rawPayload) {
  let data = null;
  ...
  const rows = [[t("verifyNumber"), data.num], [t("verifyName"), data.name || "—"]];
...
function renderDiscoverScreen() {
  ...
  return `
    <div class="discover-hero">
      ...
      <p class="subject-note">${dt("heroTagline")}</p>
    </div>

    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:8px">${dt("sectionAbout")}</h3>
      <p style="font-size:14px;line-height:1.6">${c.description}</p>
    </div>

    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:8px">${dt("sectionSubjects")}</h3>
      <ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.8">
        ${c.subjects.map((s) => `<li>${s}</li>`).join("")}
      </ul>
    </div>

    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:8px">${dt("sectionFlow")}</h3>
      <p style="font-size:14px;line-height:1.6">${c.courseFlow}</p>
    </div>

    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:12px">${dt("sectionPlans")}</h3>
      ${c.plans.map((p) => `
        <div style="padding:12px 0;border-bottom:1px solid var(--color-surface-2)">
          <div style="display:flex;justify-content:space-between;font-weight:600">
            <span>${p.name}</span><span>${p.amount} ${p.currency} ${p.frequency}</span>
          </div>
    ...
    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:10px">${dt("sectionSchedule")}</h3>
      <div style="display:flex;flex-direction:column;gap:8px">
        ${c.scheduleSlots.map((s) => `
          <div style="display:flex;align-items:center;gap:8px;font-size:14px">
            <span style="color:${s.available ? "var(--color-success)" : "var(--color-ink-soft)"}">${s.available ? "●" : "○"}</span>
            ${s.label}
          </div>
        `).join("")}
      </div>
    ...
    <div class="card" style="margin-bottom:18px">
      <h3 class="display" style="margin-bottom:8px">${dt("sectionRules")}</h3>
      <ul style="margin:0;padding-left:18px;font-size:14px;line-height:1.8">
    ...
    <div class="card" style="margin-bottom:22px">
      <h3 class="display" style="margin-bottom:10px">${dt("sectionFaq")}</h3>
      ${c.faq.map((item) => `
    ...
    <button class="btn gold block" id="discoverRegisterBtn" style="margin-bottom:12px">📝 ${dt("ctaRegister")}</button>
    ${STATE.student ? `
      <button class="btn secondary block" id="discoverShareBtn" style="margin-bottom:12px">📤 ${dt("shareViaWhatsapp")}</button>
      <button class="btn secondary block" id="discoverCopyBtn">🔗 ${dt("copyLink")}</button>
...
function attachDiscoverScreenHandlers() {
  ...
  document.getElementById("discoverShareBtn")?.addEventListener("click", () => {
    const text = `${STATE.settings.payment.academyName || t("appName")} — ${dt("heroTagline")}\n${shareUrl}`;
    window.open(whatsappLink("", text), "_blank");
  });
  document.getElementById("discoverCopyBtn")?.addEventListener("click", async () => {
    try { await navigator.clipboard.writeText(shareUrl); toast(dt("linkCopied")); }
...
function renderDiscoverRegisterForm() {
  ...
  return `
    <div class="session-header">
      <h2 class="display">${dt("formTitle")}</h2>
    ...
    <div class="field" style="margin-bottom:12px">
      <label>${dt("childAge")}</label>
      <input id="regChildAge" type="number" min="3" max="99" />
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("parentName")}</label>
      <input id="regParentName" />
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("parentPhone")}</label>
      <input id="regParentPhone" type="tel" placeholder="+221 77 000 00 00" />
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("parentEmail")}</label>
      <input id="regParentEmail" type="email" />
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("desiredPlan")}</label>
      <select id="regPlan">
        <option value=""></option>
        ${c.plans.map((p) => `<option value="${p.name}">${p.name} (${p.amount} ${p.currency} ${p.frequency})</option>`).join("")}
      </select>
    </div>
    <div class="field" style="margin-bottom:12px">
      <label>${dt("desiredSlot")}</label>
      <select id="regSlot">
        <option value=""></option>
        ${c.scheduleSlots.filter((s) => s.available).map((s) => `<option value="${s.label}">${s.label}</option>`).join("")}
      </select>
    </div>
    <div class="field" style="margin-bottom:18px">
      <label>${dt("message")}</label>
    ...
    <button class="btn gold block" id="submitRegistrationBtn">${dt("submitRegistration")}</button>
  `;
}
function attachDiscoverRegisterHandlers() {
  document.getElementById("submitRegistrationBtn").addEventListener("click", async () => {
    ...
    if (!childName || !parentPhone) { toast(dt("childName") + " / " + dt("parentPhone") + " ?"); return; }
    const reg = {
      id: newId("reg"),
    ...
    openModal(`
      ...
      <div class="empty-state" style="padding:10px 0 4px">
        <div class="star8 done" style="margin:0 auto 14px">✓</div>
        <h2 class="display">${dt("registrationSent")}</h2>
        <p>${dt("registrationSentMsg")}</p>
        <a class="btn gold block" style="margin-top:16px;text-decoration:none" href="${whatsappLink(waNumber, waText)}" target="_blank">📤 WhatsApp</a>
