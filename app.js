...
function renderSettings() {
  return `
    ...
    <div style="display:flex;flex-direction:column;gap:10px;margin-bottom:20px">
      <button class="btn secondary block" data-go="#/schedule">📅 ${t("scheduleManageBtn")}</button>
    </div>
  ...
  return `
    <div dir="${isRtl ? "rtl" : "ltr"}" class="${isRtl ? "rtl-content" : ""}">
      ...
      <div class="field" style="margin-bottom:12px">
        ...
        <select id="regPlan">
          <option value=""></option>
          ${countryData.plans.map((p) => `<option value="${p.name}">${p.name} (${p.amount} ${countryData.currency} ${p.frequency})</option>`).join("")}
        </select>
      </div>
      <div class="field" style="margin-bottom:12px">
...
function renderScheduleScreen() {
  ...
  const viewToggleHtml = `
    <div style="display:flex;gap:8px;margin-bottom:16px">
      <button class="btn ${viewMode === "day" ? "gold" : "secondary"}" data-schedule-view="day" style="flex:1;padding:9px;font-size:13px">${t("scheduleViewDay")}</button>
      <button class="btn ${viewMode === "week" ? "gold" : "secondary"}" data-schedule-view="week" style="flex:1;padding:9px;font-size:13px">${t("scheduleViewWeek")}</button>
    </div>
  `;
        ...
        return `
          <div class="card" style="margin-bottom:12px">
            ...
            ${daySlots.length === 0 ? `<div style="font-size:12.5px;color:var(--color-ink-soft)">${t("scheduleNoBookingsDay")}</div>` : daySlots.map((s) => `
              <div style="display:flex;justify-content:space-between;gap:10px;padding:7px 0;border-bottom:1px solid var(--color-surface-2);cursor:pointer" data-open-student="${s.studentId}">
                <span style="font-size:13px;font-weight:600">${s.startTime}-${s.endTime} · ${s.studentName}</span>
    ...
    ${timeline.map((item) => item.type === "busy" ? `
      <div class="card" style="margin-bottom:10px;border-left:5px solid var(--color-danger)">
        <div style="display:flex;justify-content:space-between;align-items:flex-start">
          <div style="cursor:pointer" data-open-student="${item.slot.studentId}">
...
function openScheduleSlotModal(existingSlot, preset, resuming) {
  ...
  document.getElementById("confirmSlotBtn").addEventListener("click", async () => {
    const studentId = document.getElementById("slotStudent").value;
    ...
    const toSave = {
      ...slot,
      studentId, studentName: student ? student.fullName : slot.studentName,
