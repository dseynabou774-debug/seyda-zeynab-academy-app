...
async function loadStudentProfileData(studentId) {
  ...
  const [allProgress, memorization, notes, quranEntries, topicEntries, alphabetEntries, scheduleSlots, courseLogs, diplomas] = await Promise.all([
  ...
  return { progressMap, memorization, notes, quranEntries, topicEntries, alphabetEntries, scheduleSlots, courseLogs, diplomas };
}
...
function openTopicTrackingModal(studentId, subjectKey, entry) {
  ...
  openModal(`
    ...
    <div class="field" style="margin-bottom:12px"><label>${t("topicThemeLabel")}</label><input id="tpTheme" value="${e.theme}" placeholder="${subjectKey === "athkar" ? t("topicAthkarPlaceholder") : subjectKey === "fiqh" ? t("topicFiqhPlaceholder") : t("topicTafsirPlaceholder")}" /></div>
