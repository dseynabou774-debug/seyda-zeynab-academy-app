...
async function buildDetailedReportCanvas(opts) {
  ...
  const sections = [
    ...
    { label: L.siraSection, items: opts.siraItems },
  ];
...
async function generateDetailedReport(studentId, fromISO, toISO, lang) {
  ...
  const canvas = await buildDetailedReportCanvas({
    ...
    quranItems, athkarItems: byTopic("athkar"), fiqhItems: byTopic("fiqh"), tafsirItems: byTopic("tafsir"), siraItems: byTopic("sira"),
  });
